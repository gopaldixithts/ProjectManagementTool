/****************************************************************************
 * Name : ESResourceManagementTab
 * Description : Shows the ES Resource management component for tab
 *****************************************************************************/
import { LightningElement, wire } from 'lwc';

//fetch data using apex
import getData from '@salesforce/apex/ES_ResourceManagementCtrl.getResourceAllocations';
import getFilters from '@salesforce/apex/ES_ResourceManagementCtrl.getFilters';
...
import ALLOCATION_OBJECT from '@salesforce/schema/ES_Resource_Allocation__c';
...
export default class ESResourceManagementTab extends LightningElement {
    ...
    //Fetch user record to get ES Department value for the logged in user
    ...
    setFilters(result){
        ...
        if(data){
            ...
            var defaultDept = this.ESDepts[0]?this.ESDepts[0].value:'';
            //Set the ES dept to user's dept if present else the first value
        ...
        if (result.data) {
            ...
            result.data.resourceAvailabilities.forEach(allocation => {
                ...
                MONTHS.forEach(month => {
                    cloneObj[month + CELLSTYLE_FIELD] = cloneObj[month + MONTH_REMAINING_SUFFIX] < 0 ? ES_ERROR_STYLE : BLANK_STRING;
