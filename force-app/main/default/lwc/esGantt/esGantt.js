...
import fetchGanttData from '@salesforce/apex/ES_GanttCtrl.fetchGanttData';
...
export default class ESGantt extends NavigationMixin(LightningElement) {
        ...
        setDateHeaders() {
                try {
                        ...
                        Array.from(
                                this.template.querySelectorAll("c-ES-ganttdata")
                        ).forEach(resource => {
        ...
        processData(data) {
                ...
                if (this.expandedData.length != 0 && this.InspectionId != '' && !this.isExpandedCheck && this.dataToShow.length != 0) {
                        if (this.mytaskflag == true) {
                                var filtered = [];
                                this.dataToShow.forEach(Inspection => {
                                        filtered.push(Inspection.id);
                                        if (typeof Inspection.lstOfChilds != 'undefined') {
                                                Inspection.lstOfChilds.forEach(phase => {
        ...
        handleValueChange(event) {
                ...
                switch (event.target.name) {
                        case 'projtgl':
                                ...
                                this.InspectionCheck = event.target.checked;
                                break;
                        case 'tasktgl':
                        ...
                        case 'followtgl':
                                ...
                                this.followInspection = event.target.checked;
                                break;
                }
        ...
        filteredView(cloneData) {
                ...
                cloneData.forEach(element => {
                        if (element.objAPIName == 'ES_Phase__c' || element.objAPIName == 'ES_Task__c') {
