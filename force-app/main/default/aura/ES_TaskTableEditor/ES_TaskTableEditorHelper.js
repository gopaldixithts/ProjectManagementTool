({
    ...
    cloneTask: function(component, taskRec) {
        if(taskRec) {
            var createESTaskEvent = $A.get("e.force:createRecord");
            createESTaskEvent.setParams({
                "entityApiName": "ES_Task__c",
                "defaultFieldValues": {
            ...
            createESTaskEvent.fire();
        }
    },
    
    submitESTaskForApproval: function(component, recId) {
        if(recId) {
            ...
            action.setCallback(this,function(response){
                ...
                var state = response.getState(); 
                if(state === "SUCCESS"){
                    this.showMessage("", "ES Task was submitted for Approval.", "Success");
                    $A.get('e.force:refreshView').fire();
                } else if(state ==='ERROR'){            
                    var errors = response.getError();
                    var message = '';                                  
    ...
    changeStatus: function(component, tasksSelected) {
        ...
        flow.startFlow("ES_Mass_update_Task_Status", inputVariables);
    },
        ...
        action.setCallback(this,function(response){
            ...
            if (state === "SUCCESS") {
                ...
                var inputVariables = [
                    ...
                    {
                        name : "LsESTasksSelected",
                        type : "SObject",
                        value :component.get("v.tasksToUpdate")
                    }
                ];
                // In that component, start your flow. Reference the flow's API Name.
                flow.startFlow("ES_Mass_update_Task_Assignee",inputVariables); 
            }else if(state ==='ERROR'){            
                var errors = response.getError();
                var message = '';                                  
                if (errors && Array.isArray(errors) && errors.length > 0) {
